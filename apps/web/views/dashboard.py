...
class Dashboard(View):
    def get(self, request):
        tasks = (
            Task.objects.filter(executor=Task.Executor.FORM)
        ...
        task_records = (
            TaskRecord.objects.filter(task__executor=Task.Executor.FORM, task__user__is_staff=False)
            .values("status")
            .order_by()
            .annotate(count=Count("id"))
        )
        ...
        top_users = (
            Task.objects.filter(user__is_staff=False)
